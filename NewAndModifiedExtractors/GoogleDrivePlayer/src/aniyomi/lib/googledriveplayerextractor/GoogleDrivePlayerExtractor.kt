...
class GoogleDrivePlayerExtractor(
    ...
    private fun ensureProxy(): GoogleDriveProxyServer = proxyServer ?: synchronized(this) {
        proxyServer ?: GoogleDriveProxyServer(0, proxyClient).also {
            it.start()
            if (verbose) ExtLog.d(tag, "Proxy started on port ${it.port}")
            proxyServer = it
        }
    }
    ...
    fun videosFromUrl(origRequestUrl: String): List<Video> {
        if (verbose) ExtLog.d(tag, "Fetching videos from: $origRequestUrl")
            ...
            newView.webViewClient =
                object : WebViewClient() {
                    override fun onPageFinished(
                        ...
                        if (verbose) ExtLog.d(tag, "Page loaded")
                    }
                    ...
                    override fun shouldInterceptRequest(
                        ...
                        if (verbose) ExtLog.d(tag, "Intercepted URL: $url")
                        if (VIDEO_REGEX.containsMatchIn(url) && playbackUrl == null) {
                            playbackUrl = url
                            if (verbose) ExtLog.d(tag, "Found playback URL: $url")
                            latch.countDown()
                        } else if (DOWNLOAD_REGEX.containsMatchIn(url) && downloadUrl == null) {
                            downloadUrl = url
                            if (verbose) ExtLog.d(tag, "Found download URL: $url")
                            latch.countDown()
                        }
        ...
        val rawCookieStr =
            CookieManager
                ...
                ?: ""
        if (verbose) ExtLog.d(tag, "Raw cookies len=${rawCookieStr.length}")
        ...
        if (playbackUrl == null) {
            ...
            if (downloadUrlFinal == null) {
                if (verbose) ExtLog.d(tag, "No playback or download URL intercepted")
                return emptyList()
            }
            val resolved = resolveDownloadStream(downloadUrlFinal, rawCookieStr)
            if (resolved == null) {
                if (verbose) ExtLog.d(tag, "Download URL could not be resolved to a video stream")
                return emptyList()
            }
            if (verbose) ExtLog.d(tag, "Resolved download stream: ${resolved.url.take(120)}")
            val streamId =
                proxy.register(
        ...
        if (verbose) ExtLog.d(tag, "Playback URL: $playbackUrlFinal")
        ...
        return try {
            val responseBody =
                client
                    ...
                    .execute()
                    .bodyString()
            if (verbose) ExtLog.d(tag, "Response body: ${responseBody.take(300)}...")

            if (responseBody.contains("\"error\"") || responseBody.contains("RESOURCE_EXHAUSTED")) {
                if (verbose) ExtLog.d(tag, "Playback API returned an error, falling back to download")
                ...
                if (fileId != null) {
                    ...
                    if (resolved != null) {
                        if (verbose) ExtLog.d(tag, "Download fallback resolved: ${resolved.url.take(120)}")
                        val streamId =
                            proxy.register(
                                GoogleDriveProxyServer.StreamInfo(
                                    videoUrl = resolved.url,
                ...
                if (verbose) ExtLog.d(tag, "Download fallback failed, returning empty")
                return emptyList()
            }

            if (verbose) ExtLog.d(tag, "Streaming response parsed, body len=${responseBody.length}")
            ...
            if (!hbResponse.isNullOrBlank()) {
                if (verbose) ExtLog.d(tag, "Parsing serializedHouseBrandPlayerResponse (len=${hbResponse.length})")
                try {
                    ...
                    if (hbStreaming != null) {
                        ...
                        if (verbose) ExtLog.d(tag, "HouseBrand: ${hbStreaming.formats.size} formats, ${hbStreaming.adaptiveFormats.size} adaptive")
                        ...
                        for (fmt in allFormats) {
                            ...
                            val proxyUrl = "http://127.0.0.1:${proxy.port}/stream/$streamId"
                            if (verbose) ExtLog.d(tag, "HB: itag=$itag quality=$quality proxy=$proxyUrl")
                            videos.add(Video(proxyUrl, label, proxyUrl, Headers.headersOf(), audioTracks = audioTracks))
                        }
                    }
                } catch (e: Exception) {
                    ExtLog.e(tag, "Failed to parse HouseBrand response, falling back", e)
                }
            }

            if (videos.isEmpty()) {
                if (verbose) ExtLog.d(tag, "Falling back to formatStreamingData")
                ...
                if (fmtData != null) {
                    fmtData.progressiveTranscodes.forEach { tc ->
                        ...
                        val proxyUrl = "http://127.0.0.1:${proxy.port}/stream/$streamId"
                        if (verbose) ExtLog.d(tag, "Progressive: itag=${tc.itag} quality=$quality proxy=$proxyUrl")
                    ...
                    fmtData.adaptiveTranscodes.forEach { tc ->
                        if (tc.transcodeMetadata.mimeType.startsWith("video/")) {
                            ...
                            val proxyUrl = "http://127.0.0.1:${proxy.port}/stream/$streamId"
                            if (verbose) ExtLog.d(tag, "Adaptive: itag=${tc.itag} quality=$quality proxy=$proxyUrl")
                            videos.add(
            ...
            if (verbose) ExtLog.d(tag, "Found ${videos.size} video(s)")
            videos
        } catch (e: Exception) {
            ExtLog.e(tag, "Error fetching streaming data", e)
            emptyList()
        }
    }
    ...
    private fun resolveDownloadStream(downloadUrl: String, cookies: String): ResolvedStream? {
        ...
        if (cookies.isNotEmpty()) {
            if (verbose) ExtLog.d(tag, "Cookie-based resolution failed, retrying without cookies")
    ...
    private fun resolveDownloadStreamAttempt(downloadUrl: String, cookies: String): String? {
        ...
        fun contentTypeOf(url: String): Pair<Int, String> {
            ...
            client.newCall(request).execute().use { resp ->
                ...
                if (verbose) ExtLog.d(tag, "HEAD $url -> ${resp.code} type=$type")
        ...
        if (directType.startsWith("video/")) {
            if (verbose) ExtLog.d(tag, "Download URL returns video directly ($directType)")
            return downloadUrl
        }
        ...
        val page =
            runCatching {
                client.newCall(GET(downloadUrl, baseHeaders)).execute().use { resp ->
                    if (verbose) ExtLog.d(tag, "GET $downloadUrl -> ${resp.code} final=${resp.request.url}")
                    resp.body?.string() ?: ""
                }
            }.getOrDefault("")
        val title = Regex("<title>([^<]*)</title>").find(page)?.groupValues?.get(1)
        if (verbose) ExtLog.d(tag, "Page len=${page.length} title=${title ?: "unknown"}")
        ...
        if (verbose) ExtLog.d(tag, "confirm=${confirm ?: "null"} uuid=${uuid ?: "null"}")
        ...
        if (confirm != null && uuid != null) {
            ...
            if (confirmedType.startsWith("video/")) {
                if (verbose) ExtLog.d(tag, "Resolved via confirm+uuid ($confirmedType)")
                return finalUrl
            }
            if (verbose) ExtLog.d(tag, "Confirmed HEAD not video: code=$code type=$confirmedType")
        } else {
            val finalUrl = "$downloadUrl${sep}confirm=${confirm ?: "t"}"
            val (code, type) = runCatching { contentTypeOf(finalUrl) }.getOrDefault(-1 to "")
            if (type.startsWith("video/")) {
                if (verbose) ExtLog.d(tag, "Resolved via confirm=t only ($type)")
                return finalUrl
            }
            if (verbose) ExtLog.d(tag, "confirm-only HEAD not video: code=$code type=$type")
        }
        ExtLog.w(tag, "Could not resolve download URL as video stream")
        return null
    }
