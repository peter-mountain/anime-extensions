...
class M3u8Integration(
    ...
    private val serverManager: M3u8ServerManager = M3u8ServerManager(client, fallbackClient, dns),
) {
    ...
    private fun processM3u8Video(originalVideo: Video): Video {
        ...
        val cookies = originalVideo.headers?.get("Cookie")
        val processedUrl = serverManager.processM3u8Url(originalVideo.url, referer, userAgent, cookies)
        return Video(
