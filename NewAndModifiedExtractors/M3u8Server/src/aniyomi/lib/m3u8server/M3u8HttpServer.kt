...
class M3u8HttpServer(
    ...
    private fun extractHeadersFromSession(
        ...
        if (!fallbackCookies.isNullOrBlank() && !headers.containsKey("cookie")) {
            headers["cookie"] = fallbackCookies
        }

        if (!headers.containsKey("origin") && headers.containsKey("referer")) {
            val ref = headers["referer"] ?: ""
            if (ref.startsWith("https://") || ref.startsWith("http://")) {
                val slashIdx = ref.indexOf("/", 8)
                if (slashIdx > 0) headers["origin"] = ref.substring(0, slashIdx)
            }
        }
    ...
    fun createLocalUrl(
        ...
        if (!cookies.isNullOrBlank()) {
            sb.append("&cookies=").append(URLEncoder.encode(cookies, Charsets.UTF_8.name()))
        }
        return sb.toString()
    }
            ...
            when {
                ...
                line.startsWith("#") || line.isBlank() -> {
                    ...
                    if (resolvedUrl.contains(".m3u8", ignoreCase = true)) {
                        ...
                        modifiedLines.add(buildChildM3u8Url(serverPort, resolvedUrl, referer, userAgent, cookies))
                    } else {
                        modifiedLines.add(
                            createLocalSegmentUrl(serverPort, resolvedUrl, currentKey, referer, userAgent, cookies),
                        )
                        segmentCount++
                    }
                    segmentSequence++
